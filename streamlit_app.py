        ...
        if simulation_type == "Simulation simple":
            ...
            if st.button("▶️ Lancer la Simulation simple"):
                ...
                for index, row in edited_df.iterrows():
                    ...
                    new_qte = col6.number_input("Qté Sim 1", min_value=0, value=row["Qté Sim 1"], key=f"qte_sim1_{index}", on_change=update_quantity, args=(edited_df, index))
            ...
            if objectif > 0:
                if st.button("▶️ Lancer la Simulation avec objectif de montant"):
                    ...
                    for index, row in edited_df_sim2.iterrows():
                        ...
                        col4.write(row["Stock"])
                        col5.write(row["Total ventes N-1 (sélection)"])
                        new_qte = col6.number_input("Qté Sim 2", min_value=0, value=row["Qté Sim 2"], key=f"qte_sim2_{index}", on_change=update_quantity, args=(edited_df_sim2, index))
...
def update_quantity(df, index):
    new_qte = st.session_state[f"qte_sim1_{index}"] if "qte_sim1" in st.session_state else st.session_state[f"qte_sim2_{index}"]
    if "Qté Sim 1" in df.columns:
        df.at[index, "Qté Sim 1"] = new_qte
        df.at[index, "Montant Sim 1"] = new_qte * df.at[index, "Tarif d'achat"]
    elif "Qté Sim 2" in df.columns:
        df.at[index, "Qté Sim 2"] = new_qte
        df.at[index, "Montant Sim 2"] = new_qte * df.at[index, "Tarif d'achat"]
